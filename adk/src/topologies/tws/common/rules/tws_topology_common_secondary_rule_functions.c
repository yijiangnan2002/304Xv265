...
rule_action_t ruleTwsTopSecPeerLostFindRole(void)
{
    ...
    TWSTOP_SECONDARY_RULE_LOG("ruleTwsTopSecPeerLostFindRole, run as out of case, or in case with peer profile retention enabled but lid is open");
    return rule_action_run;
}
...
rule_action_t ruleTwsTopSecNoRoleIdle(void)
{
    ...
    if ((TwsTopology_IsDfuInCase() ||  TwsTopology_PeerProfilesToRetain() != 0) && 
        appPhyStateGetState() == PHY_STATE_IN_CASE)
    {
        /* ToDo: Need to decide on the scope to clear it. */
        TWSTOP_SECONDARY_RULE_LOG("ruleTwsTopSecNoRoleIdle, ignore as either in-case DFU pending or peer profile retention enabled");
        return rule_action_ignore;
    }
