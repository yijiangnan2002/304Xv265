...
const goal_entry_t goals[] =
{
    ...
    GOAL(tws_topology_goal_select_le_adv_params, tws_topology_procedure_select_le_adv_params,
            &proc_select_le_adv_params_fns, tws_topology_goal_none),
...
void TwsTopology_HandleGoalDecision(Task task, MessageId id, Message message)
{
    ...
    {
        DEBUG_LOG_VERBOSE("TwsTopology_HandleGoalDecision. Flushed id MESSAGE:0x%x (enum:tws_topology_stopping_state_t:%d)",
                                id, td->stopping_state);
        return;
    }

    DEBUG_LOG_INFO("TwsTopology_HandleGoalDecision id MESSAGE:0x%x", id);
    ...
    switch (id)
    {
        ...
        case TWSTOP_PRIMARY_GOAL_SELECT_LE_ADV_PARAMS:
            GoalsEngine_ActivateGoal(td->goal_set, tws_topology_goal_select_le_adv_params, task, id, message, sizeof(LE_ADV_PARAMS_SET_TYPE_T));
			break;

        case TWSTOP_PRIMARY_START_WATCHDOG:
        case TWSTOP_SECONDARY_START_WATCHDOG:
        {
        ...
        case TWSTOP_PRIMARY_STOP_WATCHDOG:
        case TWSTOP_SECONDARY_STOP_WATCHDOG:
            Watchdog_Stop(&topology_watchdog);
            break;

        case TWSTOP_PRIMARY_GOAL_DISCONNECT_LRU_HANDSET:
            GoalsEngine_ActivateGoal(td->goal_set, tws_topology_goal_disconnect_lru_handset, task, id, message, 0);
            break;

        default:
            DEBUG_LOG("TwsTopology_HandleGoalDecision, unknown goal decision MESSAGE:0x%x", id);
            break;
    }
