...
void Kymera_EnableCvcPassthroughInChain(kymera_chain_handle_t chain_containing_cvc)
{
    if(chain_containing_cvc != NULL)
    {
        ...
        PanicFalse(GET_OP_FROM_CHAIN(cvc_op, chain_containing_cvc, OPR_CVC_RECEIVE));
...
void kymera_EnableCvcPassthroughMode(void)
{
    ...
    if( appKymeraGetState() == KYMERA_STATE_SCO_ACTIVE )
    {
        ...
        Kymera_EnableCvcPassthroughInChain(sco_chain);
    }
}
