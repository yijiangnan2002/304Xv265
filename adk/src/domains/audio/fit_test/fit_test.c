...
static bool fitTest_RestartPrompt(void);
...
static void fitTest_OutputConnectingIndication(output_users_t connecting_user, output_connection_t connection_type)
{
            ...
            ((connecting_user & output_user_a2dp) != output_user_a2dp))
    {
        ...
        DEBUG_LOG("fitTest_OutputConnectingIndication FIT TEST is running %d ",FitTest_IsRunning());
        FitTest_IsRunning()?fitTest_HandleEvent(fit_test_event_abort):fitTest_HandleEvent(fit_test_event_disable);
    }
}
...
static void fitTest_OutputDisconnectingIndication(output_users_t disconnected_user, output_connection_t connection_type)
{
    ...
    if(!Kymera_OutputIsChainInUse() && !FitTest_IsRunning())
    {
...
static void fitTest_HandlePromptEndInd(void)
{
    ...
    {
        fitTestGetTaskData()->prompt_count = fitTestGetTaskData()->prompt_count-1;
        DEBUG_LOG("fitTest_HandlePromptEndInd %d prompt count remaining", fitTestGetTaskData()->prompt_count);
        if(fitTestGetTaskData()->prompt_count)
        {
            DEBUG_LOG("SENDING fit_test_event_restart_prompt");
            fitTest_HandleEvent(fit_test_event_restart_prompt);
        }
        else
        {
...
static bool fitTestStateRunningHandleEvent(fit_test_event_t event)
{
    ...
    switch(event)
    {
        ...
        case fit_test_event_restart_prompt:
            result=fitTest_RestartPrompt();
        break;
...
static bool fitTest_RestartPrompt(void)
{
    DEBUG_LOG_ALWAYS("fitTest_RestartPrompt");
    KymeraFitTest_StartOnlyPrompt();
    return TRUE;
}
...
static void fitTest_EnableAncWithFitTestMode(void)
{
    ...
    /* Adaptive ANC enable and putting into gentle mute */
    msg.in_ear = (appPhyStateGetState()==PHY_STATE_IN_EAR);
