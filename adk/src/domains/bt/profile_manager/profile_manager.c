...
static bool profileManager_FindClientSendConnectCfm(Task task, task_list_data_t *data, void *arg)
{
    ...
    if (data->ptr == params->device)
    {
        ...
        DEBUG_LOG("profileManager_FindClientSendConnectCfm toTask=%x result enum:profile_manager_request_cfm_result_t:%d", task, params->result);
...
static bool profileManager_FindClientSendDisconnectCfm(Task task, task_list_data_t *data, void *arg)
{
    ...
    if (data->ptr == params->device)
    {
        ...
        DEBUG_LOG("profileManager_FindClientSendDisconnectCfm toTask=%x result enum:profile_manager_request_cfm_result_t:%d", task, params->result);
...
static bool profileManager_NextProfile(device_t device, profile_manager_request_type_t type)
{
    ...
    if (profileManager_IsRequestComplete(profile))
    {
        ...
        profileManager_SendConfirm(device, type, result);
    }
    is_success = TRUE;
...
static void profileManager_UpdateProfileRequestStatus(device_t device, uint32 profile, profile_request_status_t status, profile_manager_request_type_t type)
{
    ...
    else
    {
        ...
        if (type == profile_manager_connect)
        {
            ...
            /* Check if there is more profiles to be connected despite one of the requested profiles failed to connect */
            TaskList_IterateWithDataRawFunction(req_task_list, profileManager_RequestNextProfileOnIndication, &params);
        }
        else
        {
