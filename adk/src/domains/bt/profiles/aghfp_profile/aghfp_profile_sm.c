...
static void aghfpProfileSm_EnterConnectedIdle(aghfpInstanceTaskData* instance, voice_source_t source)
{
    ...
    if (instance->bitfields.in_band_ring && instance->sco_sink)
    {
        AghfpAudioDisconnect(instance->aghfp);
    }
}
...
{
    DEBUG_LOG("aghfpProfileSm_EnterConnectedActive(%p) enum:voice_source_t:%d", instance, source);
    ...
    */
    if (!instance->bitfields.in_band_ring && !instance->sco_sink)
    {
        AghfpAudioConnect(instance->aghfp, instance->sco_supported_packets, &audio_params);
    }
}
...
{
    DEBUG_LOG("appAgHfpExitConnectedActive(%p) enum:voice_source_t:%d", instance, source);
...
{
    DEBUG_LOG("aghfpProfileSm_EnterConnectedIncoming(%p) enum:voice_source_t:%d", instance, source);
