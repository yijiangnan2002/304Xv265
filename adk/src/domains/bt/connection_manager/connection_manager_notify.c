...
static void conManagerMsgParamsUpdateInd(const tp_bdaddr *tpaddr, uint16 conn_interval, uint16 conn_latency)
{
    ...
    if(BtDevice_GetPublicAddress(&tpaddr->taddr, &message->tpaddr.taddr))
    {
        message->tpaddr.transport = tpaddr->transport;
    }
    else
    {
        message->tpaddr = *tpaddr;
...
{
    tp_bdaddr tpaddr_to_send;

    if(BtDevice_GetPublicAddress(&tpaddr->taddr, &tpaddr_to_send.taddr))
    {
        tpaddr_to_send.transport = tpaddr->transport;
    }
    else
    {
        tpaddr_to_send = *tpaddr;
    }
    ...
    switch (notify_message)
    {
        ...
        case cm_notify_message_connected_outgoing:
            conManagerMsgConnectionInd(&tpaddr_to_send, TRUE, reason);
            conManagerMsgTpConnectionInd(&tpaddr_to_send, 
                                         notify_message == cm_notify_message_connected_incoming);
            break;
        case cm_notify_message_disconnected:
            conManagerMsgConnectionInd(&tpaddr_to_send, FALSE, reason);
            conManagerMsgTpDisconnectionInd(&tpaddr_to_send, reason);
            break;
        case cm_notify_message_disconnect_requested:
            conManagerMsgTpDisconnectRequestedInd(&tpaddr_to_send);
            break;
