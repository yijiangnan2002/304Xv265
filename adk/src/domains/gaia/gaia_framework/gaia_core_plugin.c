...
static void gaiaCorePlugin_SendUserFeatureResponse(GAIA_TRANSPORT *t, bool next_cmd, uint8 type, uint16 offset)
{
    ...
    if (Gaia_TransportGetInfo(t, GAIA_TRANSPORT_PAYLOAD_SIZE, &value))
    {
        uint16 payload_size = (uint16)value * sizeof(uint8);
        uint8 *response = (uint8*) PanicNull(malloc(payload_size));
        ...
        DEBUG_LOG_INFO("gaiaCorePlugin_SendUserFeatureResponse: (payload_size:%u, type:%02X, offset:%04X)", payload_size, (uint8)type, offset);
...
static void gaiaCorePlugin_GetUserFeature(GAIA_TRANSPORT *t, uint16 payload_length, const uint8 *payload)
{
    ...
    if (payload_length == 0)
    {
        DEBUG_LOG_INFO("gaiaCorePlugin_GetUserFeature, Payload_length: %d", payload_length);
...
static void gaiaCorePlugin_GetUserFeatureNext(GAIA_TRANSPORT *t, uint16 payload_length, const uint8 *payload)
{
    if (payload_length == 3)
    {
        ...
        DEBUG_LOG_INFO("gaiaCorePlugin_GetUserFeatureNext, Payload: %02X %02X %02X", payload[0], payload[1], payload[2]);
...
static void gaiaCorePlugin_PopulateSupportedFeaturesPacketAndSend(GAIA_TRANSPORT *transport, uint8 pdu_id)
{
    /*! @todo This needs to be limited either by the implementation of zero copy sends or by limiting to something sensible like 20 bytes */
    ...
    uint16 response_payload_length;
    uint8 more_to_come = 0;

    PanicFalse(Gaia_TransportGetInfo(transport, GAIA_TRANSPORT_PAYLOAD_SIZE, &max_response_length));

    /* ensure that the maximum responce length is not greater than the uint16_max */
    if (max_response_length > 65535)
    {
        Panic();
    }

    if (number_of_required_bytes <= max_response_length)
    {
    ...
    {
        response_payload_length = max_response_length;
        more_to_come = 1;
    }
    ...
    if (response_payload_length > 0)
    {
        ...
        if (gaiaCorePlugin_GetSupportedFeaturesPayload((response_payload_length - 1), &response_payload[1]))
        {
            ...
            GaiaFramework_SendResponse(transport, GAIA_CORE_FEATURE_ID, pdu_id, response_payload_length, response_payload);
