...
static void dfu_PeerSetContextTx(dfu_context_t context);
#endif
...
static void dfu_HandleUpgradeStatusInd(const UPGRADE_STATUS_IND_T *sts)
{
    ...
    switch (sts->state)
    {
        ...
        case upgrade_state_done:
            ...
            Dfu_SetContext(DFU_CONTEXT_UNUSED);
            break;
    ...
    switch (operation->ops)
    {
        ...
        case upgrade_ops_trnsfr_complt_res_send_to_peer:
        {
            ...
            if(UPGRADE_PEER_IS_SUPPORTED)
            {
                ...
                /* Send the context to secondary device if interective commit. */
                if(!is_silent_commit && UPGRADE_PEER_IS_PRIMARY)
                {
                    dfu_PeerSetContextTx(Dfu_GetContext());
                }
            }
#endif
        }
...
static void dfu_PeerSetContextRx(dfu_peer_set_context_t *msg)
{
    ...
    Dfu_SetContext((dfu_context_t) msg->context);
}
...
static void dfu_SetGattServiceUpdateFlagForHandset(device_t device, void *data)
{
    ...
    {
        if(Device_IsPropertySet(device, device_property_gatt_server_config)
                && Device_IsPropertySet(device, device_property_first_connect_after_dfu) == FALSE)
        {
            Device_SetPropertyU8(device, device_property_first_connect_after_dfu, TRUE);
...
static void dfu_GattConnect(gatt_cid_t cid)
{
    ...
    if(device)
    {
        ...
        if(Device_IsPropertySet(device, device_property_first_connect_after_dfu))
        {
            GattServerGatt_SetServerServicesChanged(cid);
            Device_RemoveProperty(device, device_property_first_connect_after_dfu);
        }
    }
}
