...
{
    /* If the primary device is reset or host reconnects after link-loss during prim to sec
     * data (for serialized DFU), then by resume point data, we will land here. Since image copy 
     * is already completed, don't perform again. Just pass the COPY_SUCCESSFUL id
     * to UpgradeSMHandleValidated if device has been reset.
