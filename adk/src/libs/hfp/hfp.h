...
typedef enum
{
    /*! Connect with no ex features  */
    hfp_connect_ex_none         = 0,
    /*! Connect enabling QHS features */
    hfp_connect_ex_qhs          = 1<<0,
...
/*!
    ...
    @param options Additional options to set regarding the HFP connection
...
/*!
    @brief Set the bitmask of Qualcomm Highspeed Mode.

    This can only be done when there is a SCL being created

    @param priority The priority of the AG to perform this command on.
    @param qhs_mode on/TRUE or off/FALSE

    @returns TRUE if successfully set, FALSE if the bitmask had no valid
    bits set OR the HF library has an active SLC link.
*/
void HfpSetSupportedQhsMode(const bdaddr *addr, bool qhs_mode);
