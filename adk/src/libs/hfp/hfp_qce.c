...
/****************************************************************************
NAME
    HfpSetSupportedQhsMode

DESCRIPTION
    Sets link QHS mode if the link exists
...
void HfpSetSupportedQhsMode(const bdaddr *addr, bool qhs_mode)
{
    ...
    /* Set the QHS mode if their is a valid link */
    if (link)
    {        
        /* Set if link support qhs */
        link->bitfields.link_mode_qhs = qhs_mode;
    }
}
