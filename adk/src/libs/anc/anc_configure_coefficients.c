...
#ifdef ANC_UPGRADE_FILTER
static void setPathRxMixGains(Source mic_source, anc_instance_config_t * config)
{
    unsigned rxmix_select_mask = 0;
    unsigned rxmix_enable_mask = 0;
    unsigned control_mask = 0;

    ANC_ASSERT(SourceConfigure(mic_source, STREAM_ANC_RX_MIX_FFA_GAIN, config->feed_forward_a.rxmix_gain_config.gain_shift));
    ANC_ASSERT(SourceConfigure(mic_source, STREAM_ANC_RX_MIX_FFA_SHIFT, config->feed_forward_a.rxmix_gain_config.gain));

    ANC_ASSERT(SourceConfigure(mic_source, STREAM_ANC_RX_MIX_FFB_GAIN, config->feed_forward_b.rxmix_gain_config.gain_shift));
    ANC_ASSERT(SourceConfigure(mic_source, STREAM_ANC_RX_MIX_FFB_SHIFT, config->feed_forward_b.rxmix_gain_config.gain));
...
#ifdef ANC_UPGRADE_FILTER
    setPathRxMixGains(mic_source, config);
#endif
}
