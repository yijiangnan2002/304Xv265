    ...
    const DM_ULP_EXT_SCAN_REGISTER_SCANNER_CFM_T *cfm)
{
    if (state->dmExtScanLock)
    {
        ...
        if (!cfm->status)
        {
            if (task_scan_handle_index < MAX_SCAN_HANDLES - 1)
            {
    ...
    const DM_ULP_EXT_SCAN_UNREGISTER_SCANNER_CFM_T *cfm)
{
    if (state->dmExtScanLock)
    {
        ...
        while (i < task_scan_handle_index)
        {
            if (task_found)
            {
                if (i < task_scan_handle_index - 1)
                {
                    task_scan_handles[i].registering_task = task_scan_handles[i+1].registering_task;
                    task_scan_handles[i].scan_handle = task_scan_handles[i+1].scan_handle;
                }
                i++;
            }
        ...
        task_scan_handles[i].registering_task = 0;
        task_scan_handles[i].scan_handle = 0;
        task_scan_handle_index--;
...
void connectionHandleDmBleExtScanFilteredAdvReportInd(const DM_ULP_EXT_SCAN_FILTERED_ADV_REPORT_IND_T *ind)
{
    ...
    if (arr_idx)
    {
        ...
        message->adv_data_len   = SourceBoundary(ExtScanSrc);
...
void connectionHandleDmBleExtScanDurationExpiredInd(const DM_ULP_EXT_SCAN_DURATION_EXPIRED_IND_T *ind)
{
    ...
    if (task_scan_handle_index > 0)
    {
        ...
        for (i = 0; i < task_scan_handle_index; i++)
        {
            if (ind->scan_handle == task_scan_handles[i].scan_handle)
            {
                scan_handle_found = 1;
        ...
        {
            if (ind->scan_handle_unregistered)
            {

                /* Remove the task-handle association from the tracking struct
                 * and shift all remaining pairs after it one position down. */
                while (i < task_scan_handle_index - 1)
                {
                    task_scan_handles[i].registering_task = task_scan_handles[i+1].registering_task;
                    task_scan_handles[i].scan_handle = task_scan_handles[i+1].scan_handle;
                    i++;
    ...
    const DM_ULP_PERIODIC_SCAN_SYNC_TO_TRAIN_CFM_T *cfm)
{
    if (state->dmPerScanLock)
    {
        ...
        if (!cfm->status)
        {
            if (task_sync_handle_index < MAX_SYNC_HANDLES - 1)
            {
    ...
    const DM_ULP_PERIODIC_SCAN_SYNC_TERMINATE_CFM_T *cfm)
{
    if (state->dmPerScanLock)
    {
        ...
        while (i < task_sync_handle_index)
        {
            if (task_found)
            {
                if (i < task_sync_handle_index - 1)
                {
                    task_sync_handles[i].registering_task = task_sync_handles[i + 1].registering_task;
                    task_sync_handles[i].sync_handle = task_sync_handles[i + 1].sync_handle;
                    task_sync_handles[i].source = task_sync_handles[i + 1].source;
                }
                i++;
            }
        ...
        /* Finally, clear the last element and decrement the index. */
        task_sync_handles[MAX_SYNC_HANDLES - 1].registering_task = 0;
        task_sync_handles[MAX_SYNC_HANDLES - 1].sync_handle = 0;
        task_sync_handles[MAX_SYNC_HANDLES - 1].source = 0;
        task_sync_handle_index--;
    ...
    const DM_ULP_PERIODIC_SCAN_START_FIND_TRAINS_CFM_T *cfm)
{
    if (state->dmPerScanLock)
    {
        ...
        {
            if (task_scan_train_handle_index < MAX_TRAIN_SCAN_HANDLES - 1)
            {
    ...
    const DM_ULP_PERIODIC_SCAN_STOP_FIND_TRAINS_CFM_T *cfm)
{
    if (state->dmPerScanLock)
    {
        ...
        /* Remove the task-handle association from the tracking struct and shift
         * all remaining pairs after it one position down. */
        while (i < task_scan_train_handle_index)
        {
            if (task_found)
            {
                if (i < task_scan_train_handle_index - 1)
                {
                    task_scan_train_handles[i].registering_task = task_scan_train_handles[i+1].registering_task;
                    task_scan_train_handles[i].scan_handle = task_scan_train_handles[i+1].scan_handle;
                }
                i++;
            }
        ...
        /* Finally, clear the last element and decrement the index. */
        task_scan_train_handles[i].registering_task = 0;
        task_scan_train_handles[i].scan_handle = 0;
        task_scan_train_handle_index--;
...
{
    /* Remove the task-handle association from the tracking struct and shift
     * all remaining elements back one position. Finally, zero out the last
     * element. */
    if (task_sync_handle_index > 0)
    {
        uint8 i = 0;
        uint8 handle_found = 0;
        ...
        while (i < task_sync_handle_index)
        {
            ...
            else if (sync_handle == task_sync_handles[i].sync_handle)
            {
                handle_found = 1;
            }
            else (i++);
