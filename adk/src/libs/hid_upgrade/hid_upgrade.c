...
uint16 HidUpgradeSetReportHandler(uint16 report_id, uint16 data_in_size, const uint8 *data_in)
{
...
static void HidUpgradeConnect(void)
{
    ...
    /* Connect transport task and request UPGRADE_TRANSPORT_DATA_CFM messages,
       enable multiple block requests. */
    UpgradeTransportConnectRequest(&hid_upgrade_transport_task, UPGRADE_DATA_CFM_ALL, TRUE);
}
