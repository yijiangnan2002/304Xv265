...
static handset_service_state_machine_t *handsetService_GetLeSmForTypedBdAddr(const typed_bdaddr *taddr)
{
    ...
    if (!BdaddrTypedIsEmpty(taddr))
    {
        FOR_EACH_HANDSET_SM(sm)
        {
            ...
            {
                sm_match = sm;
                /* If we've a public address where previously we only knew the random, update the sm */
                if((le_taddr.type == TYPED_BDADDR_RANDOM) && (taddr->type == TYPED_BDADDR_PUBLIC))
                {
                    sm->le_addr.taddr = *taddr;
                }
                break;
            }
        }
