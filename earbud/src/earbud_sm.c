...
static void appEnterDfuCheck(void)
{
    ...
    switch(Dfu_GetRebootReason())
    {
        case REBOOT_REASON_ABRUPT_RESET:
            ...
            /* Retain handset profiles only for interactive commit. For silent commit,
             * host is not involved so no need to retain handset profiles. */
...
void appSmHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case DFU_READY_FOR_SILENT_COMMIT:
        {
            ...
            /* No need to retain handset profiles after silent commit command is received. */
            TwsTopology_EnableHandsetProfileRetention(Dfu_GetDFUHandsetProfiles(), FALSE);
